...
)
{
    ...
    if(drvIndex < DRV_USBFS_INSTANCES_NUMBER)
    {
        ...
        if(gDrvUSBGroup[drvIndex].gDrvUSBObj.inUse == false)
        {
            ...
            if(OSAL_MUTEX_Create(&pUSBDrvObj->mutexID) == (OSAL_RESULT) OSAL_RESULT_TRUE)
            {
                ...
                usb->USB_UPWRC |= USB_UPWRC_USBPWR_Msk;
                /* Setup the Hardware */
                if(pusbInit->stopInIdle)
                {
                    usb->USB_UCNFG1 |= USB_UCNFG1_PSIDL_Msk;
                }
                else
                {
                    usb->USB_UCNFG1 &= ~USB_UCNFG1_PSIDL_Msk;
                    ...
                    {
                        usb->USB_UCNFG1 |= USB_UCNFG1_AUTO_SUSP_Msk;
                    }
                    else
                    {
                        usb->USB_UCNFG1 &= ~USB_UCNFG1_AUTO_SUSP_Msk;
                ...
                address = (uint32_t) ((void *)((uintptr_t)pUSBDrvObj->pBDT));
                usb->USB_UBDTP3 = ( address >> 24U ) & 0xFFU; 
                usb->USB_UBDTP2 = ( address >> 16U ) & 0xFFU; 
                usb->USB_UBDTP1 = ( address >> 8U ) & 0xFFU; 
...
)
{
    ...
    if((object != SYS_MODULE_OBJ_INVALID) && (object < DRV_USBFS_INSTANCES_NUMBER))
    {
        ...
        if(gDrvUSBGroup[object].gDrvUSBObj.inUse == true)
        {
            ...
            usb =  (usb_registers_t*) ((uintptr_t)pUSBDrvObj->usbID);
...
)
{
    ...
    volatile usb_registers_t *usb =  (usb_registers_t * )((uintptr_t) pUSBDrvObj->usbID);
    ...
    if((handle != DRV_HANDLE_INVALID) && (handle != (DRV_HANDLE)(NULL)) && (pUSBDrvObj->isOpened == true))
    {
        ...
        if(pUSBDrvObj->operationMode == USB_OPMODE_DEVICE)
        {
            ...
            usb->USB_UOTGIE |= USB_UOTGIE_SESS_VLD_Msk;
