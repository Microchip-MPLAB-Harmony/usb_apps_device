...
static void APP_DMAC_0_U1ReceiveCompleteCallback(DMAC_TRANSFER_EVENT event, uintptr_t contextHandle)
{
    ...
    if(appData.uartObj[PORT1].uRxFilledByteCount < APP_UART_RX_BUFF_SIZE)
    {
        ...
        if(appData.uartObj[PORT1].uActualReadByteCount != 0)
        {
            ...
            DMAC_ChannelTransfer(DMAC_CHANNEL_UART1_RX, (const void *)&SERCOM0_REGS->USART_INT.SERCOM_DATA, appData.uartObj[PORT1].uHeadPtr, appData.uartObj[PORT1].uActualReadByteCount);
    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_WAIT_FOR_SERIAL_BAUD:
            ...
            if(appData.baudReceived[PORT1] == true)
            {
                ...
                {
                    TC3_Timer16bitCounterSet(0);
                    DMAC_ChannelTransfer(DMAC_CHANNEL_UART1_RX, (const void *)&SERCOM0_REGS->USART_INT.SERCOM_DATA, appData.uartObj[PORT1].uHeadPtr, APP_DMA_RX_PACKET_SIZE);
                    U1RTS_Set();
                    appData.baudReceived[PORT1] = false;
            ...
            if(appData.uartObj[PORT1].uUartRxNotScheduled == true)
            {
                if(appData.uartObj[PORT1].uRxFilledByteCount < APP_UART_RX_BUFF_SIZE)
                {
                    ...
                    if(appData.uartObj[PORT1].uActualReadByteCount != 0)
                    {
                        ...
                        if(true != DMAC_ChannelTransfer(DMAC_CHANNEL_UART1_RX, (const void *)&SERCOM0_REGS->USART_INT.SERCOM_DATA, appData.uartObj[PORT1].uHeadPtr, appData.uartObj[PORT1].uActualReadByteCount))
                        {
                            appData.uartObj[PORT1].uUartRxNotScheduled = true;
                        }
                        SYS_INT_SourceEnable(DMAC_0_IRQn);
...
bool APP_SerialBaudUpdate(uint8_t rPort)
{
    ...
    if(rPort == PORT1)
    {
        SERCOM0_USART_SerialSetup(&serialSetup, clkFrequency);
